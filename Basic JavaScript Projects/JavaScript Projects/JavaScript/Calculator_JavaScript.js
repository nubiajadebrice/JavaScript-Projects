...
function Input_Digit(digit) {
    ...
    //we are checking to see if wait_sec_operand is true and set
...
function Handle_Operator(Next_Operator) {
    ...
    //displayed on the screen to a # and then store the result in
    //calulator.first_operand if it doesnt exist already
    ...
    } else if (operator) {//checks if an op already exists
    const Value_Now = First_Operand || 0;
    //if operator exists, property lookup is performed for the operator
    //in the Perfrom_calc obkect and the function that matches the op
    //is executed
...
keys.addEventListener('click', (event) => {
    ...
    //that was clicked
    const { target } = event;
    //if the element that was clicked on is not a button, exit the func
